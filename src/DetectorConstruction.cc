...
G4VPhysicalVolume* DetectorConstruction::Construct ()
{
  ...
  G4VSolid* latGapS;
  if( !fibre_isSquare ) latGapS = new G4Tubs ("latGapS", total_radius+depth, total_radius+gap_l, 0.5*fibre_length, 0.*deg, 360.*deg) ;
  else
  {
    G4VSolid* dummyS = new G4Box ("dummyS", total_radius+gap_l, total_radius+gap_l, 0.5*fibre_length) ;
